...
bootstrap docker-containers for cli, orderer and kafka-brokers,
peers, and chaincode containers. The tool consumes a transaction yaml
file, which leverages pre-generated crypto materials (certs) used for
member identity and network authentication - in other words, the MSP
information for each member and its corresponding network entities.
It creates channels, installs chaincodes on all 4 peers, instantiates
chaincodes on mulitple channels and peers, generates unique invoke
and query transaction proposals on every peer/channel/chaincode
to verify a fully functional network, and uses multiple background
bash processes to concurrently generate transactions to verify
fabric concurrency functionality.

*Currently the crypto material is baked into the directory. However,
there may be a tool incorporated in the near future to organically
...
-  If not already done, clone the e2e_scale tool and move it to fabric/examples/e2e.

   .. code:: bash

       cd $GOPATH/src/github.com/hyperledger/fabric/examples
       git clone https://github.com/scottz64/e2e_scale.git
       mv e2e_scale e2e
...
Make sure you are in the ``fabric/examples/e2e`` directory and in your
